...
const TopBarLayout: React.FC = ({ children }) => (
  <div className="sticky top-0 z-9 flex-shrink-0 flex bg-zinc-50 border-b border-gray-200 md:bg-white md:border-0">
    {children}
  </div>
)
...
const Layout: React.FC = ({ children }) => {
  ...
  const [ accessControlCondition, setAccessControlCondition ] = useState<any>()
  ...
  const onSetAccessControlCondition = (condition: object) => {
    setAccessControlCondition(condition)
    setShowShareModal(false)
  }

  const switchFilterMessage = () => {
    ...
    <>
      ...
      <div>
        <ShareModal
          ...
          onAccessControlConditionsSelected={onSetAccessControlCondition}
        />
        <NavigationView>
          <NavigationColumnLayout>
            ...
            <NavigationPanel onConnect={handleConnect} condition={accessControlCondition} filter={filterMessage} />
            <UserMenu
              onConnect={handleConnect}
              onDisconnect={handleDisconnect}
              onShowShareModal={setShowShareModal}
              onFilterMessage={switchFilterMessage}
